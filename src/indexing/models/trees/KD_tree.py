...
class KDTreeModel():
    ...
    def build(self,points,dim,i=0):
        if len(points) > 1:
            ...
            return [
                ...
                self.build(points[half + 1:], dim, i),
                points[half]
            ]
        elif len(points) == 1:
    ...
    def get_k_nearest(self, point, k, dim, kd_node='init',return_distances=True, i=0, heap=None):
        ...
        if kd_node is not None:
            ...
            for b in [dx < 0] + [dx >= 0] * (dx * dx < -heap[0][0]): # dx*dx is r' and decide whether to check other branch or not.
                self.get_k_nearest(point, k, dim, kd_node[b],return_distances, i, heap)
        if is_root:
            neighbors = sorted((-h[0], h[1]) for h in heap)
            return neighbors if return_distances else [n[1] for n in neighbors]
    
    def get_nearest(self,point, dim, kd_node='init',return_distances=True, i=0, nearest=None):
        ...
        if kd_node is not None:
            ...
            for b in [dx < 0] + [dx >= 0] * (dx * dx < nearest[0]):
                self.get_nearest(point, dim,kd_node[b], return_distances, i, nearest)
