...
\begin{enumerate}
    ...
    % \item\textbf{Root Node}: Root node is the node with no parents and has other nodes as children. Each node can have two child nodes since it is a binary tree. 
...
For constructing the $K$D-Tree we have following constraints:
...
\subsubsection{Range Searching in $K$D-Tree}
