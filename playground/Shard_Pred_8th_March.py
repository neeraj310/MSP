...
for i in range(CellSize):
    for j in range(CellSize):
        cellMatrix[i * CellSize +
                   j][0] = in_data_arr[(KeysPerCell * CellSize * (j)) -
                                       (not (not (j % CellSize))), 0]
        cellMatrix[i * CellSize + j][2] = in_data_arr[(KeysPerCell * CellSize *
                                                       (j + 1)) - 1, 0]
        cellMatrix[i * CellSize + j][4] = i * CellSize + j
        cellMatrix[i * CellSize +
                   j][5] = (KeysPerCell * CellSize * i) + j * KeysPerCell
        cellMatrix[i * CellSize + j][6] = i + CellSize * j
        cellMatrix[i * CellSize +
                   j][7] = (i * KeysPerCell) + (KeysPerCell * CellSize * j)

# Sort Keys along y direction
for i in range(CellSize):
    in_data_arr[KeysPerCell * CellSize * i:KeysPerCell * CellSize *
                (i + 1)] = in_data_arr[(in_data_arr[KeysPerCell * CellSize *
                                                    i:KeysPerCell * CellSize *
                                                    (i + 1), 1].argsort()) +
...
for i in range(CellSize):
    for j in range(CellSize):
        cellMatrix[i * CellSize +
                   j][1] = in_data_arr[((KeysPerCell * (i - 1)) +
                                        (KeysPerCell * CellSize * j) +
                                        (KeysPerCell - 1)), 1]
        cellMatrix[i * CellSize +
                   j][3] = in_data_arr[((KeysPerCell * i) +
                                        (KeysPerCell * CellSize * j) +
                                        (KeysPerCell - 1)), 1]
        #print(((KeysPerCell*(i-1)) + (KeysPerCell*CellSize*(j)) +(KeysPerCell-1)))
        #print( ((KeysPerCell*(i)) + (KeysPerCell*CellSize*(j)) +(KeysPerCell-1)))
        cellMatrix[i * CellSize + j][8] = ((cellMatrix[i * CellSize + j][3] -
                                            cellMatrix[i * CellSize + j][1]) *
                                           (cellMatrix[i * CellSize + j][2] -
                                            cellMatrix[i * CellSize + j][0]))
...
in_data_arr = np.hstack(
    (in_data_arr, np.zeros((in_data_arr.shape[0], 1),
                           dtype=in_data_arr.dtype)))
...
for i in range(0, in_data_arr.shape[0]):
    ...
    in_data_arr[i, 3] = cellMatrix[CellIdx][7] + (
        (KeyArea / cellMatrix[CellIdx][8]) * KeysPerCell)
...
def assemble_regression_matrix(breaks, x):
    ...
    for i in range(n_segments - 1):
        A_list.append(
            np.where(x >= fit_breaks[i + 1], x - fit_breaks[i + 1], 0.0))
...
def compute_slopes(y_hat, fit_breaks, n_segments):
    slopes = np.divide(
        (y_hat[1:n_segments + 1] - y_hat[:n_segments]),
        (fit_breaks[1:n_segments + 1] - fit_breaks[:n_segments]))
...
def calc_gradient(x, alpha, fit_breaks, r_error, n_segments):
    ...
    for i in range(n_segments - 1):
        G_list.append(
            np.where(x >= fit_breaks[i + 1], x - fit_breaks[i + 1], np.inf))
    G = np.vstack(G_list)
